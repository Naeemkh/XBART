...
void rcpp_to_std_vector_double(Rcpp::DoubleVector delta, std::vector<double> &delta_std)
{
    size_t N = delta.length();
    for (size_t i = 0; i < N; i++)
    {
        delta_std[i] = delta(i);
    }
...
// [[Rcpp::plugins(cpp11)]]
// [[Rcpp::export]]
Rcpp::List XBART_multinomial_cpp(Rcpp::IntegerVector y, int num_class, arma::mat X, arma::mat Xtest, size_t num_trees, size_t num_sweeps, size_t max_depth, size_t n_min, size_t num_cutpoints, double alpha, double beta, double tau, double no_split_penality, Rcpp::DoubleVector delta, size_t burnin = 1, size_t mtry = 0, size_t p_categorical = 0, double kap = 16, double s = 4, bool verbose = false, bool parallel = true, bool set_random_seed = false, size_t random_seed = 0, bool sample_weights_flag = true)
{
    ...
    LogitModel *model = new LogitModel(num_class, tau_a, tau_b, alpha, beta, &y_size_t, &phi, delta_std);
    model->setNoSplitPenality(no_split_penality);
