...
void mcmc_loop_multinomial(matrix<size_t> &Xorder_std, bool verbose, vector<vector<tree>> &trees, double no_split_penality, std::unique_ptr<State> &state, LogitModel *model, std::unique_ptr<X_struct> &x_struct, std::vector< std::vector<double> > & phi_samples, matrix<double> &delta_draw_xinfo, std::vector< std::vector<double> > &delta_loglike);
